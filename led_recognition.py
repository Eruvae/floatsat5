...
def f(x, *args):
	alpha, G_0, g_0 = x
	z_1, z_2, z_3 = args
	return (c*z_1-(G_0+a*np.sin(alpha))/(g_0-a*np.cos(alpha)), 
		c*z_2-(G_0+a*np.sin(alpha+2./3.*np.pi))/(g_0-a*np.cos(alpha+2./3.*np.pi)),
		c*z_3-(G_0+a*np.sin(alpha+4./3.*np.pi))/(g_0-a*np.cos(alpha+4./3.*np.pi)))
