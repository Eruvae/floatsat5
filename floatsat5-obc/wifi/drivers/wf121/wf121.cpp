...
void WF121::processMsg() {
    ...
    case wifi_evt_tcpip_configuration_id:
		...
		#endif
		address = BGLIB_MSG(buffer)->evt_tcpip_configuration.address.u;
		netmask = BGLIB_MSG(buffer)->evt_tcpip_configuration.netmask.u;
		gateway = BGLIB_MSG(buffer)->evt_tcpip_configuration.gateway.u;
